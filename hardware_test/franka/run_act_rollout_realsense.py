...
from hardware_test.franka.floor_condition import (  # noqa: E402
    FLOOR_CONDITION_KEY,
    NUM_ELEVATOR_FLOORS,
...
def _floor_condition_tensor(floor_condition: np.ndarray) -> torch.Tensor:
    if not isinstance(floor_condition, np.ndarray):
        raise ValueError("floor condition must be a numpy array")
    if floor_condition.shape != (NUM_ELEVATOR_FLOORS,):
        raise ValueError(
            f"floor condition has shape {floor_condition.shape}, expected ({NUM_ELEVATOR_FLOORS},)"
        )
    if floor_condition.dtype != np.float32:
        raise ValueError(f"floor condition must be float32, got {floor_condition.dtype}")
    if not np.isfinite(floor_condition).all():
        raise ValueError("floor condition must contain only finite values")
    if not np.logical_or(floor_condition == 0, floor_condition == 1).all() or floor_condition.sum() != 1:
        raise ValueError("floor condition must be a one-hot vector")
    return torch.from_numpy(floor_condition.copy())


def build_policy_observation(
    observation: dict[str, Any],
    *,
    floor_condition: np.ndarray,
    ...
    return {
        ...
        FLOOR_CONDITION_KEY: _floor_condition_tensor(floor_condition),
    }
...
def validate_policy_features(config: Any) -> None:
    ...
    expected_inputs = {
        ...
        FLOOR_CONDITION_KEY: (NUM_ELEVATOR_FLOORS,),
    }
...
def select_robot_action(
    ...
    *,
    ...
    observation = build_policy_observation(raw_observation, floor_condition=floor_condition)
    processed = bundle.preprocessor(observation)
    processed_floor = processed.get(FLOOR_CONDITION_KEY)
    if not isinstance(processed_floor, torch.Tensor):
        raise ValueError(f"processed {FLOOR_CONDITION_KEY} must be a torch.Tensor")
    if processed_floor.shape != (1, NUM_ELEVATOR_FLOORS):
        raise ValueError(
            f"processed {FLOOR_CONDITION_KEY} has shape {tuple(processed_floor.shape)}, "
            f"expected (1, {NUM_ELEVATOR_FLOORS})"
        )
    if processed_floor.dtype != torch.float32:
        raise ValueError(
            f"processed {FLOOR_CONDITION_KEY} must be torch.float32, got {processed_floor.dtype}"
        )
    if not bool(torch.isfinite(processed_floor).all()):
        raise ValueError(f"processed {FLOOR_CONDITION_KEY} must contain only finite values")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-floor", type=int, choices=TRAINED_ROLLOUT_FLOORS, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    floor_condition = encode_target_floor(args.target_floor)
    ...
    print(f"目标楼层: {args.target_floor}")
    print(f"one-hot 向量: {floor_condition.tolist()}")
